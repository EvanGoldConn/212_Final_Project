	 ...
	 public void fileWriter(){
        ...
        for(int i = 0;i < personArray.length;i++) {
            if(personArray[i] != null) {
            	System.out.println("person: "+" "+personArray[i].getFullName());
                Person p = personArray[i];
                data = data + p.getFullName() + "\n" + p.getEmail() + "\n" + p.getPassword() + "\n" + p.getYear() + "\n";
                ENode curE = p.getPersonEvent().getHead();
                for(int j = 0;j < p.getPersonEvent().getSize();j++) {
                    data += p.getPersonEvent().returnDates() + " " + curE.getEvent() + ",";  
                    curE = curE.getNext();
                }
                data += "\n";
                TNode curT = p.getTimeline().getHead();
                for(int k = 0;k < p.getTimeline().getSize();k++) {
                    data += curT.getPost() +'"'+",";
                    curT = curT.getNext();
                }
                p.getPersonFriends().emptyString();//setting friends string (in bstfriends) to empty 
                if(p.getPersonFriends().size() > 0){
                	p.getPersonFriends().printingFriends(p.getPersonFriends().root());
	                data += "\n" + p.getPersonFriends().returnString() + "\n";
            	}
            	else if(p.getPersonFriends().size() <= 0){
            		System.out.println(p.getFullName()+" "+"Has no friends!");
            		data += "\n";
            	}
			...
			if(option.equals("a") || option.equals("A")){
				...
				while(currentUser != null && loggedIn){
					...
					while(logInScreen){
						...
						mainBook.returnEvents().displayNextEvent();
					...
					while(userChoice.equals("2") && boolOption2){
						...
						while(boolUpload){
							...
							System.out.println("main date"+" "+month);
							currentUser.storeDates(month, day, year, hour, min); //for returning to data file
						...
						while(boolDownload){
							...
							currentUser.insertPersonalEvent(curDate, strEvent);
