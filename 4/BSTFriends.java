...
public class BSTFriends{
	...
	public BSTNode find(String k, BSTNode v){
		if(v==null || v.key().equals(k)){ 
			...
			System.out.println(k+" "+"k not fonud");
			return v;
		}else{
	...
	public String printingFriends(BSTNode v){
		if (v.left() != null){
			inOrder(v.left());
		}
		postfixExp += v.key()+",";
	...
	// 	if(v.left() != null) {
	// 		postOrderTrav(v.left());
	// 	}
	// 	if(v.right() != null) {
	// 		postOrderTrav(v.right());
	// 	}
	// 	postfixExp += v.key() + ",";   //  creates a string in postfix order
